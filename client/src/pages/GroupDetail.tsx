...
export function GroupDetail() {
  ...
  const handleSearchUsers = async () => {
    ...
    try {
      ...
      setSearchResults(response.data.users.filter((u: User) => !memberIds.has(u.id)));
    } catch (error) {
  ...
  const handleUpdateRole = async (userId: string, newRole: 'admin' | 'member') => {
    try {
      await groupsApi.updateMember(groupId!, userId, { role: newRole });
      await loadGroup();
    } catch (err: any) {
